...
function restoreRegistrationForm(reg){
  ...
  const aiCard=document.getElementById('aiCard');if(aiCard)aiCard.classList.remove('hidden');
  return true;
}
...
def inject_async_restore_ui(html: str) -> str:
    # Move bulk upload to the non-blocking endpoint.
    html = html.replace(
    ...
    # Other UI patches append code before </script>, so the original boot call
    # is no longer adjacent to the closing tag. Replace the final real init()
    # invocation directly before injecting the restore helper definitions.
