...
function isSystemGuidance(text){const x=String(text||'');return x.includes('AI 제안은 아이디어')||x.includes('FACT 항목')||x.includes('텍스트 AI가 연결되지 않아')||x.includes('AI 제안 호출 실패')||x.includes('상품 FACT를 먼저 사용자 확정')}
function editorGuidance(d){const runtime=(d.warnings||[]).filter(x=>isSystemGuidance(x));return `<div class="suggestion"><strong>AI 제안 편집 안내</strong><ul class="muted" style="line-height:1.7"><li>AI 제안은 확정 FACT를 바꾸지 않는 설명·마케팅 아이디어입니다. 실제 상품에 맞는 내용만 수정·삭제·채택합니다.</li><li>초록색은 확정 FACT에서 직접 가져온 항목입니다. 콘텐츠에서 쓰지 않을 경우 선택 해제할 수 있지만 원천 FACT는 바뀌지 않습니다.</li><li>체크된 항목만 텍스트 확장 상품정보로 저장됩니다. 필요 없는 항목은 비워둬도 됩니다.</li>${runtime.map(x=>`<li class="warn">${escapeHtml(x)}</li>`).join('')}</ul></div>`}
